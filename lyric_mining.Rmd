...
get_artist_albums <- function(name_artist){
  ...
  df_albums <- cbind(artist = rep(name_artist, nrow(df_albums)),
...
```{r}
df_accuracy <- cbind(df_lyric_dox[idx_test, ],
                     prediction = df_pred)
