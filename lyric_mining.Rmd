...
prep_data_set <- function(df_dataset){
  ...
  df_dtm_dataset <- as.tibble(as.matrix(dtm_dataset))
  df_dtm_dataset$artist <- df_dataset$artist

  lst_return <- list(df_dtm_dataset = df_dtm_dataset,
                     vocab = vocab)
...
t1 = Sys.time()
fit_svmLinear3 <- train(artist ~ ., data = lst_train$df_dtm_dataset, method = "svmLinear3")
