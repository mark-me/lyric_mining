...
Prune vocabulary: removing uncommon words in few documents
```{r}
...
t1 = Sys.time()
dtm_train = create_dtm(it_train, pruned_vocab)
print(difftime(Sys.time(), t1, units = 'sec'))
```
```{r}
identical(rownames(dtm_train), as.character(df_train$doc_id))

```

```{r}
df_train <- as.tibble(as.matrix(dtm_train))
df_train$artist <- df_lyric_dox$artist[train_ids] 
```

```{r Create linear SVM model}
t1 = Sys.time()
fit_svmLinear3 <- train(artist ~ ., data = df_train, method = "svmLinear3")
print(difftime(Sys.time(), t1, units = 'sec'))
```
