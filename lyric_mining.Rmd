...
get_artist_studio_albums <- function(artist_mbid){
  ...
  df_albums %<>%
    ...
    mutate(year_release = str_sub(date_release, 1, 4)) %>% 
...
```{r}
vec_artists <- c("Tindersticks", "Tom Waits", "Nick Cave and The Bad Seeds", "Vanilla Ice", "The National", "Metallica", "Queen")
artists_mbid <- sapply(vec_artists, get_artist_mbid)

df_albums <- tibble(artist = character(0),
                    album = character(0),
                    year_release = character(0))

musicbrainz_json_may_fail <- function(artist_mbid) {
  df_album <- get_artist_studio_albums(artist_mbid)
  return(df_album)
}
...
for(artist_mbid in artists_mbid){
  ...
  while( is.null(r) && attempt <= 20 ) {
    attempt <- attempt + 1
    try(
      reply <- musicbrainz_json_may_fail(artist_mbid)
    )
    if(!is.null(r)) print("That went well!")
    Sys.sleep(1)
  } 
  df_albums <- rbind(df_albums, reply)
  
}

write_csv2(df_albums, "artists_albums.csv")

table(df_albums$artist)
#df_albums <- do.call("rbind", lapply(artists_mbid, get_artist_studio_albums))
```
